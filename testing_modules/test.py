...
def renderingtest(ref_pointer, rend_pointer, word_pointer, result_file):
    ...
    for aline in clean_file:
        if aline.find(';') != -1:
            optional_list = []
            #This array has index of words with multiple renderings
      ...
      #multiple correct renderings will be inside multiple_glyph_words
            multiple_glyph_words.append(optional_list)
        aword = aline.split(',')
    ...
    wordfile_content = word_pointer.read()
    wordlist = []
    ...
    if result_list == []:
        ...
        f = 1
        a = array('i', [])
    #writing wrongly rendered words to result.txt
        for word in result_list:
            d = ref_list.index(word)
            a.append(d)
    flag = 0
    #Testing if the word has multiple correct renderings
